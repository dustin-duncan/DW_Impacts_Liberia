...
$$
$$
\mathrm{\text{Escapement:}} \\
\mathrm{Esc_t = \frac{1}{x}(B_t -H_t)} \\ 
\text{Biological Model:} \\
\mathrm{B_{t+1}^{DWF}= B_{t}^{DWF}+ \frac{\Phi+1}{\Phi}gB_{t}^{DWF}(1-\frac{B_{t}^{DWF}}{K})^\Phi-H_{t}^{DWF} + Esc_t} \\ ~ \\
\mathrm{B_{t+1}^{SSF}= B_{t}^{SSF}+ \frac{\Phi+1}{\Phi}gB_{t}^{SSF}(1-\frac{B_{t}^{SSF}}{K})^\Phi-H_{t}^{SSF} + Esc_t} \\
\text{Profit Function:} \\
