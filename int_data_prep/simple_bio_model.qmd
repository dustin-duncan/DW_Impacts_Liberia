---
title: "Spatially Disjoint Surplus Production Model"
subtitle: "Distant Water Impacts Masters Project, Bren School of Environmental Science and Management (2025)"
...
This is the script which implements the Bren School of Environmental Science and Management "Deep Water Impacts" Master's group project thesis. This script follows the implementation of a spatially disjoint Gordon-Schaefer model for Liberia's Sardinella fishery, for both Distant Water (DWF) and Small Scale Artisanal Fleets (SSF). This model was adapted from the Gordon Schaefer production model to account for the spatially variable aggregation of Sardinella, which tends to spend its juvenile life stage near-shore and migrate offshore after maturity; it also accounts for the fact that SSF can fish anywhere within Liberia's EEZ, but restricts access of DWF fishing in its Inshore Exclusion Zone (IEZ). As such, SSF effort is distributed between three zones: zone 3 (inshore; targeting juvenile fish), zone 2 (between IEZ boundary and inshore; targeting adult fish), and zone 1 (outside of IEZ; targeting adult fish). DWF effort is assumed to be exclusively in zone 1, and as such affects the catch per unit effort of SSF fishing in zone 1, as they are targeting the same portion of adult fish population. The motivation for developing and implementing this model is to identify the optimal access fee that Liberia's National Fisheries and Aquaculture Authority (NaFAA) should charge DWF to minimize the negative impacts of their activity on SSF.

Data on Sardinella harvest, and effort by fleet were provided for the years 2018-2023 by NaFAA, collected from FAO CECAF Small Pelagics Report (2019), and Rousseau et al., (2024); for raw data processing, see `/raw_data_prep/FAO_data_prep.qmd`, `/raw_data_prep/NaFAA_data_prep.qmd`, and `/raw_data_prep/Rousseau_data_prep.qmd`. These data were aggregated and used in Section 2 to estimate biological parameters for Liberia's Sardinella population, including the intrinsic growth rate of the population **r**, carrying capacity **K**, and catchability coefficient **q** using the **datalimited2** package (Free, CM, 2018). These parameters were estimated using the Bayesian Surplus Production Model function `datalimited2::bsm()`, developed by Froese et al. (2017). 

The remainder of the document defines and implements the spatially disjoint Gordon-Schaefer model, first model initial-state parameters including Effort (by fleet and zone), Initial Biomass (by zone), and carrying capacity were calibrated using Negative Log-Likelihood methods (aggregated fleet and biomass data). The model is then implemented under a 5-year time horizon and 10-year time horizon, using a non-linear optimization algorithm from the **nloptr** package, to find the optimal access fee $\alpha$ that Liberia should charge DWF fleets to maximize both the revenue of the SSF fleet and the revenue obtained by NaFAA from these fees. This is modulated by the $\beta$ term in NaFAA's benefits formula, which is a welfare weighting parameter conceptualized by Abigail Kirk to define the emphasis that NaFAA places on either revenue from DWF or SSF. A $\beta$ of 0 implies complete emphasis on revenue from DWF, whereas a $\beta$ of 1 implies complete emphasis on the benefits of SSF. The utility functions (see `/utils`) which are implemented to calibrate initial state parameters and run the optimization algorithm were developed by Dustin Duncan. 
...
## 2 - Biological parameter estimation

### 2.1 - Loading data 

```{r}
# FAO CECAF Small Pelagics Report data
...
### 2.2 - Using datalimited2 package to estimate range of q values, and explore fisheries dynamics 

```{r}
# Loading and tidying data from FAO report, using all metrics to calculate aggregated catch per year and  catch per unit effort 
spm_data = all_metrics %>% 
  group_by(Year) %>% 
  janitor::clean_names() %>%
...
# Loading NaFAA data, calculating aggregated catch per year 
spm_data2 = catch_nafaa %>%
  group_by(Year) %>% 
...
# Combining aggregated catch per year data to use with datalimited2 package 
spm_data = spm_data %>% 
  rbind(spm_data2)

# Loading data collected by Rousseau et al. (2024) on effort to create total aggregated catch and effort data by year.
spm_data3 = read.csv(here::here("data_int/data_lit/effort_Rousseau_bound_FAO_metrics.csv")) %>% 
...
bsm_out = datalimited2::bsm(year=spm_data3$year, 
                            catch=spm_data3$catch, 
                            biomass=spm_data3$cpue, 
                            btype="CPUE", 
...
## 3 - Implementation of Bio-economic model 

### 3.1 - Parameters used and formulas
...
### 3.2 - Testing Bioeconomic model function 

```{r}
# Estimates from FAO Report: r = 0.45/year, K = 100 000 tonnes, and BI/K = 60 percent, D=40%
catch_nafaa = read.csv(here::here("data_int/data_NaFAA/catch_timeseries_2018_2023.csv")) %>% 
  group_by(Year) %>% 
  summarize(Catch_tonnes = sum(Catch_tonnes)) %>% 
  filter(Year > 2018) %>%
...
### 3.3 - Initial State calibration against NaFAA data
...
#### 3.4.1 - Writing Business as Usual scenario five year time horizon plots out to plots folder
...
#### 3.4.2 - Writing Optimized scenario five year time horizon plots out to plots folder
...
#### 3.4.3 - Writing Ban scenario five year time horizon plots out to plots folder
...
plotBmsy_all <- ggplot(tenyrall) +
    geom_point(aes(x = time, y = bbmsy, color = scenario), alpha = 0.5,
              position=position_jitter(w=0.0, h=0.02)
              ) + 
    # geom_abline(aes(intercept=1, slope = 0), linetype="dashed", color="black") + 
    labs(x = "Time (years)", y = "B/BMSY") +
  ggtitle("Business as Usual B/BMSY over Time") +
  # geom_text(aes(x = 1, y =1.5, label=bmsystart)) +
...
#### 3.5.1 - Writing Business as Usual scenario ten year time horizon plots out to plots folder
...
#### 3.5.2 - Writing Optimized scenario ten year time horizon plots out to plots folder
...
#### 3.5.3 - Writing Ban scenario ten year time horizon plots out to plots folder
...
### 4.1 - Cost and Price sensitivity
...
### 4.2 - Wellfare term and Alpha sensitivity
...
* FAO. 2019. Report of the FAO/CECAF Working Group on the Assessment of Small Pelagic Fish – Subgroup South.
  Elmina, Ghana, 12-20 September 2018. Rapport du Groupe de travail FAO/COPACE sur l’évaluation des petits
  poissons pélagiques – Sous-groupe Sud. Elmina, Ghana, 12-20 septembre 2018. CECAF/ECAF Series / COPACE/PACE
  Séries No. 19/81. Rome.  
...
* Froese R, Demirel N, Coro G, Kleisner KM, Winker H (2017) Estimating fisheries reference points from catch and
  resilience. Fish & Fisheries 18(3): 506-526.
  
* Haddon M (2023). _MQMF: Modelling and Quantitative Methods in Fisheries_.
  doi:10.32614/CRAN.package.MQMF <https://doi.org/10.32614/CRAN.package.MQMF>, R package
  version 0.1.5, <https://CRAN.R-project.org/package=MQMF>.

* Iooss B, Da Veiga S, Janon A, Pujol G (2024). _sensitivity: Global Sensitivity Analysis of
  Model Outputs and Importance Measures_. doi:10.32614/CRAN.package.sensitivity
  <https://doi.org/10.32614/CRAN.package.sensitivity>, R package version 1.30.1,
  <https://CRAN.R-project.org/package=sensitivity>.
  
* Johnson S (2008). _The NLopt nonlinear-optimization package_.
  <https://github.com/stevengj/nlopt>.
  
* Oliveros-Ramos R, Shin Y (2016). “calibrar: an R package for the calibration of complex
  ecological models.” _Submitted_. R package version 0.3, <https://arxiv.org/abs/1603.03141>.
  
* Rousseau, Y., Blanchard, J. L., Novaglio, C., Pinnell, K. A., Tittensor, D. P., Watson, R. A., & Ye, Y. (2024).
  A database of mapped global fishing activity 1950–2017. Scientific Data, 11(1), 48.
  
* Vaughan D, Dancho M (2022). _furrr: Apply Mapping Functions in Parallel using Futures_.
  doi:10.32614/CRAN.package.furrr <https://doi.org/10.32614/CRAN.package.furrr>, R package
  version 0.3.1, <https://CRAN.R-project.org/package=furrr>.
...
* Wickham H, Henry L (2025). _purrr: Functional Programming Tools_.
  doi:10.32614/CRAN.package.purrr <https://doi.org/10.32614/CRAN.package.purrr>, R package
...
* Wickham H, Pedersen T, Seidel D (2025). _scales: Scale Functions for Visualization_.
  doi:10.32614/CRAN.package.scales <https://doi.org/10.32614/CRAN.package.scales>, R package
  version 1.4.0, <https://CRAN.R-project.org/package=scales>.
