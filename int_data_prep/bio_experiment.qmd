...
interaction_new = function(choice, state, time, params, indat=NULL, rtn_catch=TRUE, rtn_npv=FALSE, ban=FALSE) {
  ...
  with(as.list(c(state, params, choice)), {
    ...
    for(t in 2:time) {
      ...
      B[t] = B[t-1] + max(0.45*(B[t-1])*(1-(B[t-1]/K)), 0) - (qS*B[t-1]*e1[t-1]) - (qD*B[t-1]*E[t-1])
      
      # B1[t] <- B1[t-1] -(qS*B1[t-1]*e1[t-1]) - (qD*B1[t-1]*E[t-1]) + (recruit*chi)
      # 
      # B2[t] <- B2[t-1] -(qS*B2[t-1]*e2[t-1]) + (recruit*(1-chi))
      # 
      # spawn_bio <- B1[t-1]+B2[t-1]
      # 
      # B3[t] <- B3[t-1] + max(0.45*(spawn_bio)*(1-(spawn_bio/K)), 0) - (qS*B3[t-1]*e3[t-1]) - recruit
      # 
      # B3[t] <- B3[t-1] + max(0.45*(spawn_bio)*(1-(spawn_bio/K)), 0) - (qS*B3[t-1]*e3[t-1]) - recruit
      
      e1[t] <- (phi*(-((C1*e1[t-1])^2)+(p*qS*B[t-1]*e1[t-1])) ) + e1[t-1] 
      ...
      if(ban == TRUE) {
        ...
        E[t] <- (phi*(-((C*E[t-1])^2)+((1-choice)*p*qD*B[t-1]*E[t-1]))) + E[t-1]
      }
      
      HS[t] = (qS*B[t]*e1[t])
      
      PIS[t] <- (p*HS[t]) - (C1*e1[t])
    ...
    if(rtn_npv == TRUE) {
      ...
      return(tibble(time=t, B=B, e1=e1, E=E, HS=HS, HD=HD, PV=pv, pvPIS=pvPIS, PIS=PIS))
    }
  })
}
...
newstate = c(B1i = 3638.688, B2i = 9807.942, B3i = 5158.846,
          e1i = 9784.621, Ei = 3981.774)

upper = c(B1i = 15000, B2i = 15000, B3i = 10000,
          e1i = 25761.151, Ei = 26076.548,
          K=60000)
...
state = c(B1i = 12000, B2i = 12000, B3i = 6000,
          e1i = 15000, Ei = 5000, K=35000)

lower = c(B1i = 3639, B2i = 9808, B3i = 5159,
          e1i = 9785, Ei = 3982, K=30000)
...
spawntesttab = interaction_new(choice=0.1, state=best2$par,time=5, params=log(parameters), rtn_catch = FALSE, rtn_npv=FALSE)

ddplot(spawntesttab, bmsy=TRUE)

spawntestnpv = -interaction_new(choice=0.1, state=best2$par,time=5, params=log(parameters), rtn_catch = FALSE, rtn_npv=TRUE)
...
fiveyrbautab = interaction_new(choice=0.1, state=log(best2par),time=5, params=log(parameters), rtn_catch = FALSE, rtn_npv=FALSE, ban=FALSE)

fiveyrbaunpv = -interaction_new(choice=0.1, state=log(best2par),time=5, params=log(parameters), rtn_catch = FALSE, rtn_npv=TRUE, ban=FALSE)
...
fiveyropttab = interaction_new(choice=fiveyropt$solution, state=log(best2par), time=5, params=log(parameters), rtn_catch=FALSE, rtn_npv=FALSE)

fiveyroptnpv = -interaction_new(choice=fiveyropt$solution, state=log(best2par), time=5, params=log(parameters), rtn_catch=FALSE, rtn_npv=TRUE)

## Five year projection with a ban

fiveyrbantab = interaction_new(choice=0.1, state=log(best2par), time=5, params=log(parameters), rtn_catch=FALSE, rtn_npv=FALSE, ban=TRUE)

fiveyrbannpv = -interaction_new(choice=0.1, state=log(best2par), time=5, params=log(parameters), rtn_catch=FALSE, rtn_npv=TRUE, ban=TRUE)

ddplot2(fiveyrbautab, bmsy=TRUE)
ddplot2(fiveyropttab, bmsy=TRUE)
ddplot2(fiveyrbantab, bmsy=TRUE)
```
...
tenyrbautab = interaction_new(choice=0.1, state=log(best2par),time=10, params=log(parameters), rtn_catch = FALSE, rtn_npv=FALSE)
# 
# tenyrbautab2 = interaction_new(choice=1, state=log(best2par),time=10, params=log(parameters), rtn_catch = FALSE, rtn_npv=FALSE)
# 
...
tenyrbaunpv = -interaction_new(choice=0.1, state=log(best2par),time=10, params=log(parameters), rtn_catch = FALSE, rtn_npv=TRUE)
...
tenyropttab = interaction_new(choice=tenyropt$solution, state=log(best2par), time=10, params=log(parameters), rtn_catch=FALSE, rtn_npv=FALSE)

tenyroptnpv = -interaction_new(choice=tenyropt$solution, state=log(best2par), time=10, params=log(parameters), rtn_catch=FALSE, rtn_npv=TRUE)

## ten year projection with a ban

tenyrbantab = interaction_new(choice=0.1, state=log(best2par), time=10, params=log(parameters), rtn_catch=FALSE, rtn_npv=FALSE, ban=TRUE)

tenyrbannpv = -interaction_new(choice=0.1, state=log(best2par), time=10, params=log(parameters), rtn_catch=FALSE, rtn_npv=TRUE, ban=TRUE)
