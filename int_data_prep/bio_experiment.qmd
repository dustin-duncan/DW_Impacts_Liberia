...
### Function (also new function)

```{r}
interaction_new = function(choice, state, time, params, indat=NULL, rtn_catch=TRUE, rtn_npv=FALSE, ban=FALSE) {
  ...
  with(as.list(c(state, params, choice)), {
    ...
    for(t in 2:time) {
      ...
      spawn <- max(0.45*(B1[t-1]+B2[t-1])*(1-((B1[t-1]+B2[t-1])/K)), 0)
      
      B1[t] <- B1[t-1] +(recruit*(chi)) -(qS*B1[t-1]*e1[t-1])-(qD*B1[t-1]*E[t-1])
      
      B2[t] <- B2[t-1] + (recruit*(1-chi))-(qS*B2[t-1]*e2[t-1])
      
      B3[t] <- B3[t-1] + spawn - (qS*B3[t-1]*e3[t-1]) - recruit 
      
      prof_e1 = (phi*(-((C1*e1[t-1])^2)+(p*qS*B1[t-1]*e1[t-1])) )
      
      prof_e2 = (phi*(-((C2*e2[t-1])^2)+(p*qS*B2[t-1]*e2[t-1])))
      
      prop_transfer21 = 0.5
      
      if(prof_e2 < 0) {
      e1[t] <- prof_e1 + e1[t-1] + (-prof_e2*prop_transfer21)
      } else if(prof_e2 >= 0) {
      e1[t] <- prof_e1 + e1[t-1]
      }
      ...
      if(prof_e1 < 0) {
      e2[t] <- prof_e2  + e2[t-1] + (-prof_e1*prop_transfer12)
      } else if(prof_e1 >= 0) {
      e2[t] <- prof_e2  + e2[t-1]
      }
      
      prof_e3 = (phi*(-((C3*e3[t-1])^2)+(p*qS*B3[t-1]*e3[t-1])))
      e3[t] <- prof_e3 + e3[t-1] 
      
      # if(ban == TRUE) {
      #   E[t] = 0
      # } else if(fee > 0.9) {
      #   phit = rnorm(1000, mean = 0.5, sd = 0.12)
      #   E[t] = (phit*(-((C*E[t-1])^2)+((1-fee)*p*qD*B1[t-1]*E[t-1]))) + E[t-1]
      # } else {
...
constraint_new = function(choice, state, time, params, indat=NULL, rtn_catch=TRUE, rtn_npv=FALSE, ban=FALSE) {
  ...
  with(as.list(c(state, params, choice)), {
    ...
    for(t in 2:time) {
      ...
      conb3[t] = (HS3[t]+recruit-spawn) - B3[t]
    }
...
tester = interaction_new(choice=0.1, state=log(state),time=time, params=log(parameters), rtn_catch = FALSE, rtn_npv=FALSE)

npv=-interaction_new(choice=0.1, state=log(state),time=time, params=log(parameters), rtn_catch = FALSE, rtn_npv=TRUE)
...
spawntesttab = interaction_new(choice=0.1, state=best3$par,time=5, params=log(parameters), rtn_catch = FALSE, rtn_npv=FALSE)

ddplot(spawntesttab, bmsy=TRUE)

spawntestnpv = -interaction_new(choice=0.1, state=best3$par,time=5, params=log(parameters), rtn_catch = FALSE, rtn_npv=TRUE)
...
# best3par = c(B1i=15000, B2i=15000, B3i=7133, e1i=12186, e2i=16437, e3i=2193, Ei=3982,K=53855)
# best3par = c(B1i=14120, B2i=11827, B3i=6009, e1i=20630, e2i=25241, e3i=7016, Ei=5023, K=37600) 
# best3par=c(B1i=15000, B2i=11524, B3i=6009, e1i=19884, e2i=23843, e3i=7014, Ei=5024, K=37600)
# best3par=c(B1i=13755, B2i=11621, B3i=6724, e1i=23920, e2i=24550, e3i=8225, Ei=6838, K=37600)
# best3par=c(B1i=13259, B2i=11205, B3i=6684, e1i=23950, e2i=25354, e3i=8289, Ei=13605, K=53855)
# best3par=c(B1i=15000, B2i=12828, B3i=6343, e1i=17393, e2i=18150, e3i=7352, Ei=12516, K=53855)
# best3par=c(B1i=15000, B2i=12819, B3i=6357, e1i=15971, e2i=16957, e3i=7305, Ei=10000, K=53855) 
best3par = best3$par
best3par['K'] = log(53855)
best3par = exp(best3par)
best3par=best3par
## Five year projection under business as usual

fiveyrbautab = interaction_new(choice=0.1, state=log(best3par),time=5, params=log(parameters), rtn_catch = FALSE, rtn_npv=FALSE, ban=FALSE)

fiveyrbaunpv = -interaction_new(choice=0.1, state=log(best3par),time=5, params=log(parameters), rtn_catch = FALSE, rtn_npv=TRUE, ban=FALSE)
...
fiveyropt=nloptr(x0 = 0.5,
                    eval_f = interaction_new,
                    opts = options,
                    lb = 0,
                    ub = 1,
...
fiveyropttab = interaction_new(choice=fiveyropt$solution, state=log(best3par), time=5, params=log(parameters), rtn_catch=FALSE, rtn_npv=FALSE)

fiveyroptnpv = -interaction_new(choice=fiveyropt$solution, state=log(best3par), time=5, params=log(parameters), rtn_catch=FALSE, rtn_npv=TRUE)

## Five year projection with a ban

fiveyrbantab = interaction_new(choice=0.1, state=log(best3par), time=5, params=log(parameters), rtn_catch=FALSE, rtn_npv=FALSE, ban=TRUE)

fiveyrbannpv = -interaction_new(choice=0.1, state=log(best3par), time=5, params=log(parameters), rtn_catch=FALSE, rtn_npv=TRUE, ban=TRUE)
...
tenyrbautab = interaction_new(choice=0.1, state=log(best3par),time=10, params=log(parameters), rtn_catch = FALSE, rtn_npv=FALSE)
# 
# tenyrbautab2 = interaction_new(choice=1, state=log(best3par),time=10, params=log(parameters), rtn_catch = FALSE, rtn_npv=FALSE)
# 
# eff_diff = tibble(time=tenyrbautab1$time, E_low=tenyrbautab1$E, E_high=tenyrbautab2$E, H_low=tenyrbautab1$HD, H_high=tenyrbautab2$HD)
# 
# ggplot(eff_diff) + 
#   geom_line(aes(x=time, y=E_low), color="red") + 
#   geom_line(aes(x=time, y=E_high), color="green")
# ggplot(eff_diff) + 
#   geom_line(aes(x=time, y=H_low), color="red") + 
#   geom_line(aes(x=time, y=H_high), color="green")

tenyrbaunpv = -interaction_new(choice=0.1, state=log(best3par),time=10, params=log(parameters), rtn_catch = FALSE, rtn_npv=TRUE)
...
tenyropttab = interaction_new(choice=tenyropt$solution, state=log(best3par), time=10, params=log(parameters), rtn_catch=FALSE, rtn_npv=FALSE)

tenyroptnpv = -interaction_new(choice=tenyropt$solution, state=log(best3par), time=10, params=log(parameters), rtn_catch=FALSE, rtn_npv=TRUE)

## ten year projection with a ban

tenyrbantab = interaction_new(choice=0.1, state=log(best3par), time=10, params=log(parameters), rtn_catch=FALSE, rtn_npv=FALSE, ban=TRUE)

tenyrbannpv = -interaction_new(choice=0.1, state=log(best3par), time=10, params=log(parameters), rtn_catch=FALSE, rtn_npv=TRUE, ban=TRUE)
...
npv_v_C1_p <- ggplot(both_cost, aes(C1,npv, color = p)) +
  geom_point(alpha = 0.7) + 
    ...
    theme_bw() +
    labs(x = "C1", y = "NPV", color = "p") + 
    ggtitle("NPV versus C1 across varying p") + 
    theme(axis.title = element_text(size = 12, face = "bold"))
...
npv_v_p_C1 <- ggplot(both_cost, aes(p,npv, color = C1)) +
  geom_point(alpha = 0.7) +
    ...
    theme_bw() +
    labs(color = "C1", y = "NPV", x = "p")+ 
    ggtitle("NPV versus p across varying C1") + 
    theme(axis.title = element_text(size = 12, face = "bold"))
