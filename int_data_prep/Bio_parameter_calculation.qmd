...
negLL(pars=log(inp), funk=aspm, logobs=log(catch_nafaa[, "catch"]$catch), indat=catch_nafaa, inparams=inparams)
...
# npar <- length(inp)
# negLL(inp, aspm, log(catch_nafaa[, "catch"]$catch))
...
bestL1 <- optim(inp2,negLL,method="Nelder-Mead",
               funk=aspm,
               logobs=log(catch_nafaa[, "catch"]$catch),
               indat=catch_nafaa, inparams=inparams,
               control=list(maxit=1000,parscale=scalepar))
